...
def _final_install_test() -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="evalflow-launch-install-") as tmp:
        ...
        if version.returncode != 0 or "> evalflow v0.1.2" not in version.stdout:
...
def _git_release(tag_release: bool, push_tag: bool) -> str:
    if not tag_release:
        return "Release tagging skipped. Re-run with --tag-release to create v0.1.2."

    git_dir = ROOT / ".git"
    if not git_dir.exists():
        raise RuntimeError("No .git directory found; run release tagging from a real git clone")

    tag_result = _run(["git", "tag", "v0.1.2"], cwd=ROOT)
    if tag_result.returncode != 0 and "already exists" not in tag_result.stderr:
        raise RuntimeError(f"git tag failed:\n{tag_result.stdout}\n{tag_result.stderr}")

    if not push_tag:
        return "Created or verified local tag v0.1.2. Re-run with --push-tag to publish it."

    push_result = _run(["git", "push", "origin", "v0.1.2"], cwd=ROOT, timeout=240)
    if push_result.returncode != 0:
        raise RuntimeError(f"git push failed:\n{push_result.stdout}\n{push_result.stderr}")
    return "Created and pushed git tag v0.1.2."
...
def main() -> int:
    ...
    parser.add_argument(
        "--tag-release",
        action="store_true",
        help="Create the local git tag v0.1.2 after all checks pass.",
    )
    parser.add_argument(
        "--push-tag",
        action="store_true",
        help="Push the v0.1.2 tag to origin after all checks pass.",
    )
